...
/* =================================================================== */
   ...
   int PPIDD_Get(int handle,int64_t ilo,int64_t ihi,void *buff) {
      ...
      if ( mpiga_inquire_storetype(handle) == 0 )
         ...
         if ( (mpiilo==mpiihi) && (dtype==MPI_INT||dtype==MPI_LONG||dtype==MPI_INT32_T||dtype==MPI_INT64_T) ) { /* PPIDD_helpga_get_inum */
   ...
   int PPIDD_Put(int handle,int64_t ilo,int64_t ihi,void *buff) {
      ...
      if ( mpiga_inquire_storetype(handle) == 0 )
         ...
         int ielem=mpiilo;
         MPI_Datatype dtype=twosided_helpga_inquire_dtype(handle);
         if ( (mpiilo==mpiihi) && (dtype==MPI_INT||dtype==MPI_LONG||dtype==MPI_INT32_T||dtype==MPI_INT64_T) ) { /* PPIDD_helpga_put_inum */
